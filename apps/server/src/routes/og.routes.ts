...
import { idWhere } from '../services/og/idToken';
...
router.get('/athlete/:id.png', browseLimiter, async (req: Request, res: Response) => {
  try {
    const where = idWhere(req.params.id as string);
    if (!where) { await sendFallback(res); return; }
    // take:2 — a 12-hex slug token is a prefix, not guaranteed unique; >1 match
    // is ambiguous → treat as not-found (mirrors the public SSR resolver).
    const rows = await prisma.user.findMany({
      where,
      select: { id: true, name: true, sport: true, position: true, location: true, verified: true, age: true, role: true, discoverable: true, guardianManaged: true },
      take: 2,
    });
    ...
    // Key career totals (no photo — minor-safety; monogram instead). Query by the
    // RESOLVED id — the request may have addressed the athlete by slug token.
    let stats: { value: string; label: string }[] = [];
    try {
      if (user!.sport === 'FOOTBALL') {
        const a = await prisma.footballStats.aggregate({ where: { userId: user!.id }, _count: { _all: true }, _sum: { goals: true, assists: true } });
        ...
        const a = await prisma.basketballStats.aggregate({ where: { userId: user!.id }, _count: { _all: true }, _sum: { points: true, rebounds: true } });
...
router.get('/tournament/:id.png', browseLimiter, async (req: Request, res: Response) => {
  try {
    ...
    const rows = await prisma.tournament.findMany({
      ...
      take: 2, // ambiguous slug-token prefix → not-found
    });
    const t = rows.length === 1 ? rows[0] : null;
...
router.get('/team/:id.png', browseLimiter, async (req: Request, res: Response) => {
  try {
    ...
    const team = rows.length === 1 ? rows[0] : null;
