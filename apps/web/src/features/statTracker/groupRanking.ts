...
//
// Mirror of server/src/services/groupRanking.ts. The two MUST agree: the server
// ranks the public group tables and seeds the knockout from them, this one ranks
// the tracker's live tables and the offline demo. A disagreement would show an
// organiser one qualifier and give the bracket another.
//
...
// tied — recomputed over only the games between the teams that remain. That is
// what the recursion below is doing.
// ─────────────────────────────────────────────────────────────────────────────

/** The standings fields ranking needs. */
 ...
 * figures. BOTH basketball codes — the blowout argument applies to a game played
 * to 21 exactly as it does to one played to 80, and FIBA 3x3 breaks ties this
 * way too.
 */
...
/**
 * League-table points for one result, by discipline. Mirror of the server's
 * resultPoints — the two MUST agree or the tracker's live table and the public
 * one will disagree about who is top of the group.
 *
 * FIBA 3x3 pays TWO for a win and ONE for a loss: a side that turns up and loses
 * must finish above one that forfeits, which 3-1-0 cannot express. Every side
 * that plays therefore gains points, so the points column separates teams less
 * and the head-to-head tiebreak does more of the work.
 */
...
function headToHeadOrder<T extends RankableRow>(tied: T[], matches: RankableMatch[]): T[] {
  ...
  return [...tied].sort(
    (a, b) => b.goalDifference - a.goalDifference
      ...
      // table that reshuffles itself between two reads would be worse.
...
export function orderGroup<T extends RankableRow>(
  ...
  if (!usesHeadToHeadTiebreak(discipline)) return sorted;
