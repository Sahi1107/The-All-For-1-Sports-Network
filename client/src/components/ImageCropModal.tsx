      ...
      <div className="bg-[#111] border border-ink/10 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            <ZoomOut size={14} className="text-foreground/40" />
            <input
              type="range"
              min={1}
              max={3}
            ...
            />
            <ZoomIn size={14} className="text-foreground/40" />
          </div>

          {/* Rotation */}
          <div className="flex items-center gap-3">
            <RotateCw size={14} className="text-foreground/40" />
            <input
              type="range"
              min={0}
              max={360}
              ...
              className="flex-1 accent-primary h-1"
            />
            <span className="text-xs text-foreground/40 w-8 text-right">{rotation}°</span>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 border border-ink/10 text-foreground/70 hover:text-foreground rounded-lg text-sm transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex-1 py-2.5 bg-primary hover:bg-primary-dark disabled:opacity-40 text-on-primary font-semibold rounded-lg text-sm transition-colors flex items-center justify-center"
            >
              {saving
